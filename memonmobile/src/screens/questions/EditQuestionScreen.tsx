...
const EditQuestionScreen = () => {
  ...
  // Get question data from route params
  const question: ResponseQuestion | undefined = route.params?.question;

  // Form state - initialized with existing question data
  const [questionName, setQuestionName] = useState(question?.questionName || '');
  const [difficulty, setDifficulty] = useState(question?.difficulty || '');
  const [description, setDescription] = useState(question?.formData?.description || '');
  const [tags, setTags] = useState<string[]>(question?.tags || []);
  const [options, setOptions] = useState<string[]>(
    question?.formData?.options?.length > 0 ? question.formData.options : ['']
  );
  const [link, setLink] = useState(question?.formData?.link || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // TODO: Call your API to update the question
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <Surface style={[styles.header, { backgroundColor: colors.surface }]} elevation={0}>
        ...
        />
        <Text style={[styles.headerTitle, { color: colors.onSurface }]}>Edit Question</Text>
        <View style={{ width: 40 }} />
      </Surface>
